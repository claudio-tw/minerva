...
def transform_original_dataset(dataset: pd.DataFrame):
    ...
    cat_cols = df.select_dtypes(include=[int, np.int32, np.int64]).columns.tolist()
    baseline = df[cat_cols].min()
    assert not baseline.isna().any()
    df[cat_cols] -= baseline
    cat_cols = df.select_dtypes(include=[int, np.int32, np.int64, 'object']).columns.tolist()
    if 'label' in cat_cols:
        cat_cols.remove('label')
    float_cols = df.select_dtypes(include=[float, np.float32, np.float64]).columns.tolist()
    ...
    assert set(targets).intersection(set(float_cols).union(set(cat_cols))) == set()
    # 8. Rename features with muted names
    assert len(set(constants.train_cat_cols)) == len(constants.train_cat_cols)
    assert len(set(constants.train_float_cols)) == len(constants.train_float_cols)
    assert set(constants.train_cat_cols).intersection(set(constants.train_float_cols))  == set()
    assert set(constants.train_cat_cols).issubset(set(cat_cols))
    assert set(constants.train_float_cols).issubset(set(float_cols))
    df = df.reindex(columns = constants.train_cat_cols + constants.train_float_cols + ['label'])
    cat_cols = df.select_dtypes(include=[int, np.int32, np.int64, 'object']).columns.tolist()
    if 'label' in cat_cols:
        cat_cols.remove('label')
    if 'y' in cat_cols:
